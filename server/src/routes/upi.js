...
// Create a UPI intent URI for direct app launch (merchant VPA must be configured)
router.post('/intent', async (req, res) => {
  try {
    const { name, mobile } = req.body;
    if (!name || !mobile) return res.status(400).json({ error: 'Missing' });

    const amount = 1.00; // INR
    const receipt = `rcpt_upi_${Date.now()}`;
    ...
    const upiUri = `upi://pay?${params.toString()}`;
    // PhonePe deep link (best-effort) — not guaranteed across all versions
    const phonepeUri = `phonepe://upi/transfer?${params.toString()}`;
...
// Get order by id (for checking UPI txn status)
router.get('/:id', async (req, res) => {
  const order = await Order.findById(req.params.id).lean();
  if (!order) return res.status(404).json({ error: 'Not found' });
  res.json(order);
